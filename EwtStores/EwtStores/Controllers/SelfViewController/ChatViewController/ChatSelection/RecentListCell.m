...
-(void)setUnionObject:(MessageUserUnionObject*)aUnionObj
{
    ...
    if ([aUnionObj.user isKindOfClass:[UserObject class]]) {
        UserObject *user = (UserObject *)aUnionObj.user;
        
        [_userNickname setText:user.userNickname];
        //[self setHeadImage:user.userHead];
        [self setHeadImage:@"defaut_friend_online"];
    } else if ([aUnionObj.user isKindOfClass:[GroupObject class]]) {
        GroupObject *group = (GroupObject *)aUnionObj.user;
        
        [_userNickname setText:group.groupName];
        //[self setHeadImage:aUnionObj.user.userHead];
        [self setHeadImage:@"troop_default_head"];
    }
...
{
    [_userHead setImage:[UIImage imageNamed:imageURL]];
